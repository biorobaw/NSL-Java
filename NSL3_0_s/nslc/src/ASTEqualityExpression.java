...
public class ASTEqualityExpression extends ASTExpression {
    ...
    public String getNslElemEquHeader(boolean isNsl, String temp) {
        String assign = (isNsl ? ".setReference(" : " = (");
        String get = (isNsl ? ".get(), " : ", ");
        return temp + assign + "NslEqu.eval(" + temp + get;
    }

    public String getNslElemNeqHeader(boolean isNsl, String temp) {
        String assign = (isNsl ? ".setReference(" : " = (");
        String get = (isNsl ? ".get(), " : ", ");
        return temp + assign + "NslNeq.eval(" + temp + get;
    }

    public String getEquType(NslScope scope, String type1, String type2) {
        return "boolean";
    }

    public String getNeqType(NslScope scope, String type1, String type2) {
        return getEquType(scope, type1, type2);
    }

    public boolean canApplyElemEqu(int dim1, int dim2) {
        return dim1 == dim2 || dim1 == 0 || dim2 == 0;
    }

    public boolean canApplyElemNeq(int dim1, int dim2) {
        return canApplyElemEqu(dim1, dim2);
    }

    public boolean canApplyElemEqu(String type1, String type2) {
        return ((type1.toLowerCase().contains("boolean")) && (type2.toLowerCase().contains("boolean")));
    }

    public boolean canApplyElemNeq(String type1, String type2) {
        return canApplyElemEqu(type1, type2);
    }

    public String getElemEquType(NslScope scope, String type1, String type2) {
    ...
    public String getElemNeqType(NslScope scope, String type1, String type2) {
        ...
        for (int i = 1; i < childNum; i++) {
            ...
            if (operator.equals("==")) {
                ...
                if (((isArray1 || isArray2) && !canApplyElemEqu(type1, type2))) {
                    NslCompiler.printError("ASTEqualityExpression", "Trying to use the \"==\" operator with non boolean types: " + type1 + " and " + type2, oper.getFirstToken().beginLine, oper.getFirstToken().beginColumn);
                    return "";
                } else if (((isArray1 || isArray2) && canApplyElemEqu(dim1, dim2))) {
                    type1 = getElemEquType(scope, type1, type2);
                    dim1 = scope.getDim(type1);
                    isArray1 = dim1 > 0;
                    mults = new StringBuilder(getNslElemEquHeader(scope.isNslType(type1), scope.getTempName(type1)) + mults + ", " + oper.getExpression() + "))");
                } else if (isArray1 || isArray2) {
                    NslCompiler.printError("ASTEqualityExpression", "Trying to compare wrong dimensions", oper.getFirstToken().beginLine, oper.getFirstToken().beginColumn);
                    return "";
                } else {
                    // Scalar equality
                    ...
                    // Simple equality
                    type1 = getEquType(scope, type1, type2);
                    //}
                    dim1 = 0;
                    isArray1 = false;
                ...
                if (((isArray1 || isArray2) && !canApplyElemNeq(type1, type2))) {
                    ...
                    dim1 = scope.getDim(type1);
                    isArray1 = dim1 > 0;
                    mults = new StringBuilder(getNslElemNeqHeader(scope.isNslType(type1), scope.getTempName(type1)) + mults + ", " + oper.getExpression() + "))");
                } else if (isArray1 || isArray2) {
                    NslCompiler.printError("ASTEqualityExpression", "Trying to compare wrong dimensions", oper.getFirstToken().beginLine, oper.getFirstToken().beginColumn);
                    return "";
                } else {
                    ...
                    // Simple inequality
                    type1 = getNeqType(scope, type1, type2);
                    //}
                    dim1 = 0;
                    isArray1 = false;
                    mults = new StringBuilder(wrapStart + mults + ext1 + operator + oper.getExpression() + ext2 + wrapEnd);
