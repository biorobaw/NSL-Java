...
public class ASTConditionalAndExpression extends ASTExpression {
    ...
    public String getNslElemAndHeader(boolean isNsl, String temp) {
        String assign = (isNsl ? ".setReference(" : " = (");
        String get = (isNsl ? ".get(), " : ", ");
        return temp + assign + "NslAnd.eval(" + temp + get;
    }

    public String getAndType(NslScope scope, String type1, String type2) {
        return "boolean";
    }

    public String getElemAndType(NslScope scope, String type1, String type2) {
    ...
    public boolean canApplyElemAnd(int dim1, int dim2) {
        return dim1 == dim2 || dim1 == 0 || dim2 == 0;
    }

    public boolean canApplyElemAnd(String type1, String type2) {
        ...
        for (int i = 1; i < childNum; i++) {
            ...
            if (operator.equals("&&")) {
                if (((isArray1 || isArray2) && canApplyElemAnd(type1, type2))) {
                    NslCompiler.printError("ASTConditionalAndExpression", "Trying to use the \"and\" operator with non boolean types: " + type1 + " and " + type2, oper.getFirstToken().beginLine, oper.getFirstToken().beginColumn);
                    return "";
                } else if (((isArray1 || isArray2) && canApplyElemAnd(dim1, dim2))) {
                    type1 = getElemAndType(scope, type1, type2);
                    dim1 = scope.getDim(type1);
                    isArray1 = dim1 > 0;
                    mults = new StringBuilder(getNslElemAndHeader(scope.isNslType(type1), scope.getTempName(type1)) + mults + ", " + oper.getExpression() + "))");
                } else if (isArray1 || isArray2) {
                    NslCompiler.printError("ASTConditionalAndExpression", "Trying to use the \"and\" operator with wrong dimensions", oper.getFirstToken().beginLine, oper.getFirstToken().beginColumn);
                    return "";
                } else {
                    ...
                    // Simple and
                    type1 = getAndType(scope, type1, type2);
                    //}
                    dim1 = 0;
                    isArray1 = false;
