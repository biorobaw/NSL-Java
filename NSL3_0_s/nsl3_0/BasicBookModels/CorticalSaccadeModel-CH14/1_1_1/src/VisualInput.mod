/* SCCS  @(#)VisualInput.mod	1.2---09/22/99--23:21:02 */

/* VisualInput
* Visual Input
* @see VisualInput.nslm
* @version 98/6/23
...
//Saccade_visualinput
// Ssaccade generator ala Scudder;  These arrays represent the increased
...
//
// protocolNum::0. manual // same as single saccade
// delta = 0.005 =  5 msec
...
// The following are locations that light up for different protocols
// within the visual input.  We use them for doing temporal plots on
// these individual locations within the larger array visualinput.


public void initModule() {	
	visualinput.nslSetAccess('W');
	protocolNum.nslSetAccess('W');
}

public void initRun() {	
	protocolNum=-1;
	nslSetValue(protocolNum,"domineyModel.protocolNum");
	if (system.debug>1) {
...
public void simRun() {
	...
	// ("single")
	if ((protocolNum==0)||(protocolNum==1)) {
		// fixation - off
		if (currentTime>0.02){
