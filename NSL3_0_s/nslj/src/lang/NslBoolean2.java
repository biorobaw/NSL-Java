...
@SuppressWarnings({"Duplicates", "finally", "ReturnInsideFinallyBlock", "unused", "UnusedAssignment"})
public class NslBoolean2 extends NslBoolean {
    ...
    public NslData duplicateThis() {
        if (isDataSet())
            return new NslBoolean2(getboolean2());
        else return new NslBoolean2();
    }
    ...
    public String toString() {
        ...
        for (i = 0; i < size1; i++) {
            strbuf.append("{ ");
            for (j = 0; j < size2; j++)
                strbuf.append(_data[i][j]).append(" ");
            strbuf.append("} ");
        }
