...
@SuppressWarnings({"WeakerAccess", "UnusedAssignment", "Duplicates", "SameParameterValue", "unused"})
class CallFrame {
    ...
    protected Hashtable<String, Var> varTable;
	...
	if ((var.value instanceof TclObject)) {
	...
	try {
		...
		return (result[0].flags & Var.UNDEFINED) == 0;
	} catch (TclException e) {
	    throw new TclRuntimeError("unexpected TclException: " + e);
	}
    }
	...
	for (Enumeration<Var> e1 = varTable.elements();
		 e1.hasMoreElements(); ) {
	    Var v = e1.nextElement();
	...
	for (Enumeration<Var> e = varTable.elements(); e.hasMoreElements(); ) {
	    Var var = e.nextElement();
