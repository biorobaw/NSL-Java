...
@SuppressWarnings({"FieldCanBeLocal", "WeakerAccess", "unused", "UnusedAssignment", "SameParameterValue", "UnusedReturnValue", "ResultOfMethodCallIgnored"})
public class Interp {
    ...
    Hashtable<String, Command> cmdTable;
    ...
    Hashtable<String, AssocData> assocDataTab;
    ...
    Hashtable<String, PackageCmd.Package> packageTable;
    ...
    dispose() {
        ...
        if (cmdTable != null) {
            for (Enumeration<String> e = cmdTable.keys(); e.hasMoreElements(); ) {
                String cmdName = e.nextElement();
        ...
        while (assocDataTab != null) {
            Hashtable<String, AssocData> table = assocDataTab;
            assocDataTab = null;

            for (Enumeration<String> e = table.keys(); e.hasMoreElements(); ) {
                Object key = e.nextElement();
                AssocData data = table.get(key);
    ...
    */
    ...
    // cmdName.
    {
        Command oldCmd = cmdTable.get(cmdName);

        if (oldCmd instanceof CommandWithDispose) {
    ...
    {
        Command cmd = cmdTable.get(cmdName);
        if (cmd == null) {
            return -1;
        } else {
