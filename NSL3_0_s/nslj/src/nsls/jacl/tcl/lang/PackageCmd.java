...
@SuppressWarnings({"UnusedAssignment", "ConstantConditions", "ThrowFromFinallyBlock", "WeakerAccess", "SameParameterValue"})
class PackageCmd implements Command {
    ...
    pkgRequire(
        ...
        if (pkg.version == null) {
            sbuf = new StringBuffer();
            sbuf.append("can't find package ").append(pkgName);
            if (version != null) {
                sbuf.append(" ").append(version);
            }
        ...
        switch (opt) {
            case OPT_FORGET: {
                ...
                for (i = 2; i < argv.length; i++) {
                    ...
                    pkg = interp.packageTable.get(pkgName);
            ...
            case OPT_NAMES: {
                ...
                try {
                    sbuf = new StringBuffer();
                    enumer = interp.packageTable.keys();
                    once = false;
                    while (enumer.hasMoreElements()) {
                        once = true;
                        key = enumer.nextElement();
                        pkg = interp.packageTable.get(key);
            ...
            case OPT_PROVIDE: {
                ...
                if (argv.length == 3) {
                    pkg = interp.packageTable.get(argv[2].toString());
                    if (pkg != null) {
                        if (pkg.version != null) {
