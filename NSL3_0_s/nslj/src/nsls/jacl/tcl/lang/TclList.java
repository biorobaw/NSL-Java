 ...
 */
@SuppressWarnings({"WeakerAccess", "FinalPrivateMethod", "FinalStaticMethod", "SameParameterValue"})
public class TclList extends InternalRep {

    /**
     * Internal representation of a list value.
     */
    private Vector<TclObject> vector;
    ...
    protected InternalRep duplicate() {
        ...
        for (int i = 0; i < size; i++) {
            TclObject tobj = vector.elementAt(i);
     ...
     * @throws TclException if the object doesn't contain a valid list.
     */
    private static final void splitList(Interp interp, Vector<TclObject> v, String s)
            throws TclException {
        int len = s.length();
        int i = 0;
    ...
    public static TclObject[] getElements(Interp interp, TclObject tobj)
        ...
        for (int i = 0; i < size; i++) {
            objArray[i] = tlist.vector.elementAt(i);
        }
        return objArray;
    }
    ...
    public static final TclObject index(Interp interp, TclObject tobj,
        ...
        if (index < 0 || index >= tlist.vector.size()) {
            ...
            return tlist.vector.elementAt(index);
        }
    }
    ...
    public static final void replace(Interp interp, TclObject tobj, int index,
        ...
        if (index >= size) {
            ...
            for (i = 0; i < count; i++) {
                TclObject obj = tlist.vector.elementAt(index);
                obj.release();
