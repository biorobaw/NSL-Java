...
@SuppressWarnings({"unused", "UnusedAssignment", "FinalPrivateMethod", "FinalStaticMethod", "DuplicateExpressions", "SameParameterValue"})
class GlobCmd implements Command {
    ...
    cmdProc(
        ...
        try {
            if ((TclList.getLength(interp, resultList) == 0) && !noComplain) {
                ...
                for (i = firstArg; i < argv.length; i++) {
                    ret.append(sep).append(argv[i].toString());
                    if (i == firstArg) {
                        sep = " ";
                    }
                }
    ...
    doGlob(
        ...
        if (openBraceIndex != -1) {
            ...
            baseBuf.append(tail, tailIndex, openBraceIndex);
            ...
            for (pIndex = openBraceIndex;
                ...
                baseBuf.append(tail, pIndex, nextIndex);
        ...
        headBuf.append(tail, tailIndex, pIndex);
        if (pIndex < tailLen) {
        ...
        switch (JACL.PLATFORM) {
            ...
            case JACL.PLATFORM_WINDOWS:
                if (headBuf.length() == 0) {
                    if (((tail.length() > 1)
                            && (tail.charAt(0) == '\\')
                            && ((tail.charAt(1) == '/') || (tail.charAt(1) == '\\')))
                            || ((tail.length() > 0) && (tail.charAt(0) == '/'))) {
                        headBuf.append("\\");
                    } else {
                        headBuf.append(".");
                    }
