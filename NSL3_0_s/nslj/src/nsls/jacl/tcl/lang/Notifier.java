...
public class Notifier {
    ...
    private static Hashtable<Thread, Notifier> notifierTable = new Hashtable<>();

    /*
     * List of registered timer handlers.
     */

    Vector<TimerHandler> timerList;
    ...
    // or others defined by event sources.
    {
        ...
        while (true) {
            ...
            if (!timerPending && (timerList.size() > 0)) {
                TimerHandler h = timerList.elementAt(0);
    ...
    serviceIdle() {
        ...
        while (idleList.size() > 0) {
            IdleHandler h = idleList.elementAt(0);
            if (h.generation > gen) {
                break;
            }
...
class TimerEvent extends TclEvent {
            ...
            int flags)            // Same as flags passed to Notifier.doOneEvent.
    {
        ...
        while (notifier.timerList.size() > 0) {
            TimerHandler h = notifier.timerList.elementAt(0);
            if (h.generation > gen) {
                break;
            }
            if (h.atTime > sysTime) {
